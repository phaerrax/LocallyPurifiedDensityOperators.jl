...
adj(t::ITensor) = swapprime(dag(t), 0 => 1)
...
@testset "Basic linear algebra" begin
    ...
    @test inner(x, apply(u, x)) ≈ tr(apply(u, ρ)) ≈ tr(apply(ρ, u))
    @test inner(x, apply(t, x)) ≈ tr(apply(t, ρ)) ≈ tr(apply(ρ, t))

    @test norm(apply(u, x))^2 ≈ tr(apply(u, ρ; apply_dag=true))
    @test norm(apply(t, x))^2 ≈ tr(apply(t, ρ; apply_dag=true))

    @test norm(apply(adj(u), x))^2 ≈ tr(apply(ρ, u; apply_dag=true))
    @test norm(apply(adj(t), x))^2 ≈ tr(apply(ρ, t; apply_dag=true))
end
